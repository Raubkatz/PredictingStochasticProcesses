...
TRAINABLE_DEPTH_MAX = MAX_FRACTAL_DEPTH
TRAINABLE_DEPTH_INIT = float(MAX_FRACTAL_DEPTH)
...
# ============================================================
# DOW JONES DATA DOWNLOAD
# ============================================================
#
# This block intentionally follows the structure of the working asset-download
# script: a dedicated yfinance downloader with retries, curl_cffi Chrome
# impersonation, MultiIndex cleanup, Date index handling, and then a small
# loader that extracts the daily close-price series.
#
# IMPORTANT:
# The forecasting experiment below receives the Dow Jones DAILY CLOSE PRICES,
# not returns. The first value is therefore kept; no pct_change() is applied.


def download_yfinance_asset(
    ticker: str,
    start_date: str,
    end_date: str,
) -> pd.DataFrame:
    ...
    for attempt in range(1, 6):
        try:
            ...
            last_error = ValueError(f"empty dataframe for {ticker}")

        except Exception as exc:
            last_error = exc

        wait = 5 * attempt
        print(f"[retry] {ticker}: attempt {attempt}/5 failed; waiting {wait}s")
        time.sleep(wait)

    raise ValueError(f"No data downloaded for ticker: {ticker}. Last error: {last_error}")


def load_dow_jones_daily_close_2019_2025() -> Tuple[pd.DatetimeIndex, np.ndarray]:
    """
    Load Dow Jones Industrial Average daily close prices.

    Date range:
        2019-01-01 through 2025-12-31 inclusive.

    Download structure:
        This deliberately mirrors the older working downloader:
            - yf.download(...)
    ...
    Returned target series:
        Daily close/adjusted-close price level, not returns.
    """
    ticker = "^DJI"
    start_date = "2019-01-01"
    end_date = "2026-01-01"  # yfinance end is exclusive; includes 2025-12-31 if available

    raw_df = download_yfinance_asset(
        ticker=ticker,
    ...
    if price_col is None:
        raise ValueError(f"No Adj Close or Close column found for {ticker}. Columns: {list(out.columns)}")

    prices = pd.to_numeric(out[price_col], errors="coerce")
    dates = pd.to_datetime(out.index)

    keep = prices.notna() & dates.notna()
    prices = prices.loc[keep]
    dates = dates[keep]

    df = pd.DataFrame({"Date": dates, "Close": prices.astype(float).values})
    df = df.dropna(subset=["Date", "Close"])
    df = df.sort_values("Date")
    df = df.drop_duplicates(subset=["Date"], keep="last")
    df = df[(df["Date"] >= pd.Timestamp("2019-01-01")) & (df["Date"] <= pd.Timestamp("2025-12-31"))]

    if len(df) < 30:
        raise ValueError(f"Too few Dow Jones close-price rows after cleaning: {len(df)}")

    print(
        f"[ok] Dow Jones daily close prices loaded: "
        f"rows={len(df)}, first={df['Date'].min().date()}, last={df['Date'].max().date()}"
    )

    return pd.DatetimeIndex(df["Date"]), df["Close"].astype(np.float32).to_numpy()
